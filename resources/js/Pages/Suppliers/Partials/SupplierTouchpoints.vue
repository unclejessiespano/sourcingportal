...
import {ClockIcon, ExclamationCircleIcon, SignalIcon, TruckIcon, ChatBubbleLeftEllipsisIcon} from "@heroicons/vue/24/solid/index.js";
...
const props = defineProps(['activity', 'supplier'])
</script>

<template>
    <ul role="list" class="space-y-6 mx-2">
        <li v-for="(activityItem, index) in activity" :key="activityItem.id" class="relative flex gap-x-4">
            <div :class="[index === activity.length - 1 ? 'h-6' : '-bottom-6', 'absolute left-0 top-0 flex w-6 justify-center']">
                <div class="w-px bg-gray-200" />
            </div>
            <div v-if="activityItem.touchpoint.lineactivity" :class="[activityItem.touchpoint.lineactivity ? activityItem.touchpoint.lineactivity.bg_color : 'bg-white', 'relative flex rounded-full h-8 w-8 flex-none items-center justify-center']">
                <SignalIcon v-if="activityItem.touchpoint.lineactivity.icon === 'SignalIcon'" class="h-6 w-6 text-white" aria-hidden="true" />
                <TruckIcon v-else-if="activityItem.touchpoint.lineactivity.icon === 'TruckIcon'" class="h-6 w-6 text-white" aria-hidden="true" />
                <ClockIcon v-else-if="activityItem.touchpoint.lineactivity.icon === 'ClockIcon'" class="h-6 w-6 text-white" aria-hidden="true" />
                <ExclamationCircleIcon v-else-if="activityItem.touchpoint.lineactivity.icon === 'ExclamationCircleIcon'" class="h-6 w-6 text-white" aria-hidden="true" />
                <ExclamationCircleIcon v-else-if="activityItem.touchpoint.code === 'commit'" class="h-6 w-6 text-white" aria-hidden="true" />
                <ExclamationTriangleIcon v-else-if="activityItem.touchpoint.code === 'linelt'" class="h-6 w-6 text-white" aria-hidden="true" />
                <BuildingOfficeIcon v-else-if="activityItem.touchpoint.code === 'supadd'" class="h-6 w-6 text-white" aria-hidden="true" />
                <HandThumbUpIcon v-else-if="activityItem.touchpoint.code === 'commit'" class="h-6 w-6 text-white" aria-hidden="true" />
                <TruckIcon v-else-if="activityItem.touchpoint.code === 'lindlv'" class="h-6 w-6 text-white" aria-hidden="true" />
                <ChatBubbleLeftEllipsisIcon v-else-if="activityItem.touchpoint.code ==='comlin'" class="h-6 w-6 text-white" aria-hidden="true" />
            ...
            <div class="flex-auto py-0.5 text-xs leading-5 text-gray-500">
                <p><span v-if="activityItem.user" class="font-medium text-gray-900">{{ activityItem.user.name }}</span> <a :href="'/supplier/'+supplier.slug+'/'+activityItem.identifier">{{activityItem.identifier}}</a> {{ activityItem.touchpoint.touchpoint }}. <template v-if="activityItem.supplierscore">{{activityItem.supplierscore.value}}</template></p>
                <time :datetime="activityItem.created_at" class="sm:hidden flex-none py-0.5 text-xs leading-5 text-gray-500">{{ moment(activityItem.created_at).format('LLL') }}</time>
                <div v-if="activityItem.comment" class="not-prose relative bg-slate-50 rounded-xl overflow-hidden dark:bg-slate-800/25">
                    ...
                    <div class="relative rounded-xl overflow-auto p-8">
                        <div class="flex justify-normal">
                            <div v-html="activityItem.comment.comment" class="select-text inline-flex text-left bg-white text-slate-900 font-semibold font-sans text-sm shadow-lg rounded-lg ring-1 ring-slate-900/5 px-4 py-3 dark:bg-slate-800 dark:ring-0 dark:highlight-white/5 dark:text-slate-200">
                            </div>
                        </div>
                    </div>
            ...
            <time :datetime="activityItem.created_at" class="hidden sm:block flex-none py-0.5 text-xs leading-5 text-gray-500">{{ moment(activityItem.created_at).format('LLL') }}</time>
        </li>
    </ul>

    <!--
    <ul role="list" class="space-y-6">
        <li v-for="(activityItem, activityItemIdx) in activity2" :key="activityItem.id" class="relative flex gap-x-4">
            <div :class="[activityItemIdx === activity2.length - 1 ? 'h-6' : '-bottom-6', 'absolute left-0 top-0 flex w-6 justify-center']">
                <div class="w-px bg-gray-200" />
            </div>
    ...
    -->
    <!-- New comment form -->
    <div class="mt-6 mx-2 flex gap-x-3">
